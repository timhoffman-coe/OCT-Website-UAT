...
import { addNewsEditor, removeNewsEditor } from '@/lib/actions/news-permission-actions';
import { Plus, Trash2, Newspaper, ShieldCheck } from 'lucide-react';
...
export default function NewsEditorsClient({
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      try {
        const permission = await addNewsEditor(user.email);
        setEditors(prev => [
          {
            id: permission.id,
            createdAt: new Date(),
  ...
  const handleRemove = (permissionId: string) => {
    if (!confirm('Remove this user\'s news edit access?')) return;
    setError('');

    startTransition(async () => {
      try {
        await removeNewsEditor(permissionId);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 bg-purple-50 border border-purple-200 rounded-lg p-4">
        ...
        <p className="text-sm text-purple-800 font-sans">
          Super Admins automatically have news edit access and do not need to be added here.
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
          <h2 className="font-sans text-sm font-semibold text-gray-700 flex items-center gap-2">
            <Newspaper size={16} className="text-[#005087]" />
            News Editors ({editors.length})
          </h2>
        </div>

        {editors.length === 0 ? (
          <div className="p-8 text-center text-gray-500 font-sans text-sm">
            No editors added yet. Only Super Admins can currently edit news posts.
          </div>
        ) : (
